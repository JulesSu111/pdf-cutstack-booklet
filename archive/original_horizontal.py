...
def make_64up_order_stackable(n_pages: int):
    ...
    half_units = padded // 4           # 半张小册子单元总数
    half_point = half_units // 2       # 前半段 / 后半段的分界
...
def impose_64up(
    ...
    while i < len(page_order):
        ...
        for slot_idx in range(per_side):
            ...
            rot = rotate_each
            if back_rotate_180 and is_back_side:
                rot = (rot + 180) % 360
...
def main():
    ap = argparse.ArgumentParser(
        description=(
            "把单页 PDF 按 64 开方式拼到 A4 上（2x2，每张纸 8 页，带等分线，可对背面单独旋转 180°）。"
    ...
    ap.add_argument("--margin-mm", type=float, default=1.0, help="每个小页离象限边界的白边（mm）")
    ap.add_argument("--gap-mm", type=float, default=0.0,
                    help="兼容参数，不再使用，可随便填")
    ap.add_argument("--rotate", type=int, default=0,
                    help="所有小页的基础旋转角度：0 / 90 / 180 / 270")
    ap.add_argument("--back-rotate-180", action="store_true",
                    help="对每张纸的背面小页额外再旋转 180°")
    ...
    impose_64up(
        ...
        back_rotate_180=args.back_rotate_180,
